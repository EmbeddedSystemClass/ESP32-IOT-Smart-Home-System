...
typedef struct Network
{
	...
	* @param[in] network Poinmqtt_mbedtls_readter to the Network struct
...
* @return Number of bytes read, or a negative value if there was an error
*/
int esp32_read(struct Network* network, unsigned char* buffer, int len, int timeout_ms);
...
* @return Number of bytes written, or a negative value if there was an error
*/
int esp32_write(struct Network* network, unsigned char* buffer, int len, int timeout_ms);
