...
/*    if (buflen>=5 &&
    ...
    if (strstr(recv_buf, "GET ") && strstr(recv_buf, " HTTP/1.1")) {
      //printf("\nGET recv_buf: %s\n",recv_buf );
      ESP_LOGI(http_server_tag, "SSL get matched message")
      ESP_LOGI(http_server_tag, "SSL write message")
      if(strstr(recv_buf, "/ ")){
        ...
        if (err > 0) {
            ESP_LOGI(http_server_tag, "OK")
        } else {
            ESP_LOGI(http_server_tag, "error")
        }
...
void http_server_init(void){
    ...
    ret = xTaskCreate(&http_server, "http_server", 2048, NULL, 5, NULL);
    if (ret != pdPASS)  {
        ESP_LOGI(http_server_tag, "create task %s failed", OPENSSL_ESP32IOT_TASK_NAME);
    }
}